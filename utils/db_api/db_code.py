...
class ProductDB(Model):
    ...
    def revirce_image(self, out_put_image):
        with open(out_put_image, 'rb') as f:
            new_image = f.read()
            return new_image
...
class OrderDB(Model):
    ...
    def get(self, user_id, product_id):
        query = f"""SELECT * FROM orders WHERE user_id={user_id} AND product_id={product_id}"""
...
if __name__ == '__main__':
    ...
    # ProductDB().save(name="Pepperoni", price=53000,
    #                  picture=ProductDB().bimg(img_path=f'{BASE_DIR}/media/taomlar/Pepperoni_1.jpg'))
    # ProductDB().save(name="Margarita", price=42000,
    #                  picture=ProductDB().bimg(img_path=f'{BASE_DIR}/media/taomlar/Margarita_2.jpg'))
